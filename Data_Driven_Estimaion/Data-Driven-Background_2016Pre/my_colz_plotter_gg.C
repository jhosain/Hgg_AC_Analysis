 ...
 for (int ii=0;ii<nproc;ii++)
  {
   ...
   TFile *file = TFile::Open("output_"+proc_Name[ii]+"_2016PreVFP.root");
...
printf("Fake-Fake: %d\n", FFPhotons);
		 ...
		 //  gStyle->SetPaintTextFormat("4.4f");
		   ...
		   hist->GetXaxis()->SetTitle("lead photon IDMVA");
                   hist->Draw("COLZ");
                   TLatex *t2a = new TLatex(0.52,0.90,"#bf{CMS} #it{Simulation}                                 (13TeV) 2016PreVFP ");
                   t2a->SetNDC();
                   t2a->SetTextFont(42);
    ...
    lineY->Draw("same");
                   ...
                   c1->cd();
                   c1->Print("Plots/2D_photon_IDMVA_"+proc_Name[ii]+"_2016PreVFP.png");
                //   c1->Print("Plots/2D_photon_IDMVA_"+proc_Name[ii]+"_2016PreVFP.pdf");
		   c1->Draw();
   }//ii 
}
