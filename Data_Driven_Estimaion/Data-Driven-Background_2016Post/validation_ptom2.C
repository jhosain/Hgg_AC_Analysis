...
void validation_ptom2(){

 std::vector<std::string> File_Names = {
		"output_Data_2016PostVFP.root",
                "output_data_driven_bkg_2016PostVFP.root",
                "output_ggjets_2016PostVFP.root",
		"output_gjet_2016PostVFP.root",
  		"output_jj_2016PostVFP.root"
  };
  ...
  for (int i=0; i< File_Names.size(); i++){
   ...
   if(File_Names[i]== "output_Data_2016PostVFP.root") Tname = "tagsDumper/trees/Data_13TeV_VBFTag_2";
   else if (File_Names[i] =="output_ggjets_2016PostVFP.root") Tname = "tagsDumper/trees/dipho_13TeV_VBFTag_2";
   else if (File_Names[i] =="output_gjet_2016PostVFP.root") Tname = "tagsDumper/trees/gjets_13TeV_VBFTag_2";
   else if (File_Names[i] =="output_jj_2016PostVFP.root") Tname = "tagsDumper/trees/qcd_13TeV_VBFTag_2";
...
//      tree->SetBranchAddress("dijet_dphi", &dijet_dphi);
    ...
    hist_[i] = new TH1F("hist", "", 50, 0.2, 0.6 );
...
if (File_Names[i]=="output_Data_2016PostVFP.root")

{ if (dipho_leadIDMVA > -0.2 && dipho_subleadIDMVA > -0.2) hist_[i]->Fill(dipho_sublead_ptoM,1); else continue;}
                
else if (File_Names[i]=="output_data_driven_bkg_2016PostVFP.root") {hist_[i]->Fill(dipho_sublead_ptoM, wt);}

else if (File_Names[i]=="output_ggjets_2016PostVFP.root")
{ if (dipho_leadIDMVA > -0.2 && dipho_subleadIDMVA > -0.2) hist_[i]->Fill(dipho_sublead_ptoM, wt*lumi);}

// else {if (!(dipho_lead_genmatch==1 && dipho_sublead_genmatch==1) && (dipho_lead_genmatch==1 || dipho_sublead_genmatch==1) && (dipho_leadIDMVA > -0.2 && dipho_subleadIDMVA > -0.2)) {hist_[i]->Fill(dipho_sublead_ptoM, wt*lumi);}}
else if (File_Names[i]=="output_jj_2016PostVFP.root")

{if (!(dipho_lead_genmatch==1 && dipho_sublead_genmatch==1) && (dipho_leadIDMVA < -0.2 && dipho_subleadIDMVA < -0.2))
hist_[i]->Fill(dipho_sublead_ptoM, wt*lumi);}
...
else hist_[i]->Fill(dipho_sublead_ptoM, wt*lumi);
...
//  c1->SetLogy();
  ...
  TLatex *tl = new TLatex(0.53,0.9," #bf{CMS} #it{Preliminary}                          16.8 fb^{-1} (13 TeV) ");
  tl->SetNDC();
  tl->SetTextFont(42);
  tl->SetTextSize(0.04);
  tl->SetTextAlign(20);
...
//  c2->SetTopMargin(0);
  ...
  rp1->GetXaxis()->SetTitle("P_{T}^{#gamma2}/m_{#gamma#gamma}");
...
//  l1->Draw("same");
  ...
  c->Print("Plots/validation_sublead_ptoM.png");
  c->Print("Plots/validation_sublead_ptoM.pdf"); 
