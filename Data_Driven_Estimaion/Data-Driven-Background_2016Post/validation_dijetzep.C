...
//      tree->SetBranchAddress("dijet_dphi", &dijet_dphi);
    ...
    hist_[i] = new TH1F("hist", "", 50, 0.0, 6.0 );
...
{ if (dipho_leadIDMVA > -0.2 && dipho_subleadIDMVA > -0.2) hist_[i]->Fill(dijet_zep,1);}
                
else if (File_Names[i]=="output_data_driven_bkg_2016PostVFP.root") hist_[i]->Fill(dijet_zep, wt);

else if (File_Names[i]=="output_ggjets_2016PostVFP.root") 
{if (dipho_leadIDMVA > -0.2 && dipho_subleadIDMVA > -0.2) hist_[i]->Fill(dijet_zep, wt*lumi);}

//else {if (!(dipho_lead_genmatch==1 && dipho_sublead_genmatch==1) && (dipho_lead_genmatch==1 || dipho_sublead_genmatch==1) && (dipho_leadIDMVA > -0.2 && dipho_subleadIDMVA > -0.2)) {hist_[i]->Fill(dijet_zep, wt*lumi);}}
...
hist_[i]->Fill(dijet_zep, wt*lumi);}

else hist_[i]->Fill(dijet_zep, wt*lumi);
...
//  c2->SetTopMargin(0);
  ...
  rp1->GetXaxis()->SetTitle("dijet_zep");
...
//  l1->Draw("same");
  ...
  c->Print("Plots/validation_dijet_zep.png");
  c->Print("Plots/validation_dijet_zep.pdf"); 
