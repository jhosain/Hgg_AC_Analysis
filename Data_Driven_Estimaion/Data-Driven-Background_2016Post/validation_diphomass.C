...
else if (File_Names[i]=="output_data_driven_bkg_2016PostVFP.root") {hist_[i]->Fill(dipho_mass, wt);}

else if (File_Names[i]=="output_ggjets_2016PostVFP.root") { if ((dipho_leadIDMVA > -0.2 && dipho_subleadIDMVA > -0.2)) hist_[i]->Fill(dipho_mass, wt*lumi);}
...
//  c1->SetLogy();
  ...
  TLatex *tl = new TLatex(0.53,0.9," #bf{CMS} #it{Preliminary}                          16.8 fb^{-1} (13 TeV) 2016PostVFP");
  tl->SetNDC();
  tl->SetTextFont(42);
  tl->SetTextSize(0.04);
  tl->SetTextAlign(20);
...
//  l1->Draw("same");
  ...
  c->Print("Plots/validation_dipho_mass_2016PostVFP.png");
  c->Print("Plots/validation_dipho_mass_2016PostVFP.pdf"); 
