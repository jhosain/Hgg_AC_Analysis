...
void qcdfraction_EB_sub()
{


    TFile* final_file = TFile::Open("subleading_EB_PF_qcdfraction.root", "RECREATE");
    ...
    TH1F *hgg_pf = (TH1F*)fgg->Get("h_pf_EB_subleading");
    TH1F *hgg_fp = (TH1F*)fgg->Get("h_fp_EB_subleading");
    TH1F *hgj_pf = (TH1F*)fgj->Get("h_pf_EB_subleading");
    TH1F *hgj_fp = (TH1F*)fgj->Get("h_fp_EB_subleading");
    TH1F *hjj_pf = (TH1F*)fjj->Get("h_pf_EB_subleading");
    TH1F *hjj_fp = (TH1F*)fjj->Get("h_fp_EB_subleading");


    TH1* num = (TH1*)hgj_pf->Clone();
    TH1* denom = (TH1*)hgj_pf->Clone();
...
//   float ymax = num->GetMaximum();
   num->GetXaxis()->SetTitle("p_{T}^{#gamma2} [GeV]");
   ...
   TLatex *t2a = new TLatex(0.54,0.91," #bf{CMS} #it{Simulation}                                            (13 TeV) 2016PostVFP");
   t2a->SetNDC();
   t2a->SetTextFont(42);
   ...
   TLatex * text_proc2 = new TLatex(0.20,0.74,"PF#rightarrowPP");
   text_proc2->SetNDC();
...
/*
  ...
  legend1->AddEntry(num, "Subleading", "l");
  legend1->Draw();
*/

   c1->Print("Plots/subleading_EB_PF_qcdfraction_2016PostVFP.png");
   c1->Print("Plots/subleading_EB_PF_qcdfraction_2016PostVFP.pdf");

   final_file->Write();
   final_file->cd();
   num->SetName("subleading_EB_PF_qcdfraction");
   num->Write();
   final_file->Close();
