...
void fakefactor_EE_sub()
{
   ...
   TFile* final_file = TFile::Open("EE_subleading_fakefactor.root", "RECREATE");
    ...
    TH1F *hjj_pp = (TH1F*)fjj->Get("h_pp_EE_subleading");
    TH1F *hjj_fp = (TH1F*)fjj->Get("h_fp_EE_subleading");
    TH1F *hjj_pf = (TH1F*)fjj->Get("h_pf_EE_subleading");

    TH1F *hgj_pp = (TH1F*)fgj->Get("h_pp_EE_subleading");
...
//   c1->SetLogx();
 ...
 //  float ymax = num->GetMaximum();
   ...
   num->SetName("EE_subleading_fakefactor");
   
   final_file->Write();
   final_file->cd();
   num->Write();
   final_file->Close();
   c1->Print("Plots/EE_subleading_fakefactor_2016PostVFP.png");
   c1->Print("Plots/EE_subleading_fakefactor_2016PostVFP.pdf");
