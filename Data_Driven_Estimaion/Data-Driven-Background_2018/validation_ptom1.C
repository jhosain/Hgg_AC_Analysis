...
//      tree->SetBranchAddress("dijet_dphi", &dijet_dphi);
    ...
    hist_[i] = new TH1F("hist", "", 50, 0.0, 1.5 );
...
//if (!isBlindedRegion && CMS_hgg_mass > 100 && CMS_hgg_mass < 180  && dipho_lead_ptoM > 0.33 && dipho_sublead_ptoM > 0.25 && dijet_leadPt > 40.0 && dijet_subleadPt > 30.0 && fabs(dijet_leadEta) < 4.7 && fabs(dijet_subleadEta) < 4.7 && fabs(dipho_leadEta) < 2.5 && fabs(dipho_subleadEta) < 2.5)

  {

if (File_Names[i]=="output_Data_2018.root")

{ if (dipho_leadIDMVA > -0.2 && dipho_subleadIDMVA > -0.2) hist_[i]->Fill(dipho_lead_ptoM,1); else continue;}
                
else if (File_Names[i]=="output_data_driven_bkg_2018.root") {hist_[i]->Fill(dipho_lead_ptoM, wt);}

else if (File_Names[i]=="output_ggjets_2018.root") 

{if (dipho_leadIDMVA > -0.2 && dipho_subleadIDMVA > -0.2) hist_[i]->Fill(dipho_lead_ptoM, wt*lumi);}

//else {if (!(dipho_lead_genmatch==1 && dipho_sublead_genmatch==1) && (dipho_lead_genmatch==1 || dipho_sublead_genmatch==1) && (dipho_leadIDMVA > -0.2 && dipho_subleadIDMVA > -0.2)) {hist_[i]->Fill(dipho_lead_ptoM, wt*lumi);}}
...
hist_[i]->Fill(dipho_lead_ptoM, wt*lumi);}
...
else hist_[i]->Fill(dipho_lead_ptoM, wt*lumi);
...
//  c2->SetTopMargin(0);
  ...
  rp1->GetXaxis()->SetTitle("P_{T}^{#gamma1}/m_{#gamma#gamma}");
...
//  l1->Draw("same");
  ...
  c->Print("Plots/validation_dipho_lead_ptoM.png");
  c->Print("Plots/validation_dipho_lead_ptoM.pdf"); 
