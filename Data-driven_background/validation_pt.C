...
//       if ( !isBlindedRegion && dipho_leadIDMVA > -0.9 && dipho_subleadIDMVA > -0.9)
 
{
     if (CMS_hgg_mass > 100 && CMS_hgg_mass < 180 && dipho_lead_ptoM > 0.33 && dipho_sublead_ptoM > 0.25 && dijet_leadPt > 40.0 && dijet_subleadPt > 30.0 && fabs(dijet_leadEta) < 4.7 && fabs(dijet_subleadEta) < 4.7 && fabs(dipho_leadEta) < 2.5 && fabs(dipho_subleadEta) < 2.5 && dipho_leadIDMVA > -0.9 && dipho_subleadIDMVA > -0.9) 
           {

     if (File_Names[i]=="output_Data_2017.root")
                        {if ( !isBlindedRegion && dipho_leadIDMVA > -0.2 && dipho_subleadIDMVA > -0.2) hist_[i]->Fill(dipho_mass); }

     else  if (File_Names[i]=="output_data_driven_bkg_2017.root")
                        {if ( !isBlindedRegion && dipho_leadIDMVA > -0.2 && dipho_subleadIDMVA > -0.2) continue;
                        else if ( !isBlindedRegion && dipho_leadIDMVA > -0.2 && dipho_subleadIDMVA < -0.2) hist_[i]->Fill(dipho_mass, wt);
                        else if ( !isBlindedRegion && dipho_leadIDMVA < -0.2 && dipho_subleadIDMVA > -0.2) hist_[i]->Fill(dipho_mass, wt);
                        else if ( !isBlindedRegion && dipho_leadIDMVA < -0.2 && dipho_subleadIDMVA < -0.2) hist_[i]->Fill(dipho_mass, wt); }

//    else {if (  dipho_leadIDMVA > -0.2 && dipho_subleadIDMVA > -0.2) hist_[i]->Fill(dijet_Mjj, wt*lumi);}
    else if (File_Names[i]=="output_ggjets_2017.root")
{ if (!isBlindedRegion)hist_[i]->Fill(dipho_mass, wt*lumi);}

else {if (!isBlindedRegion  && (dipho_lead_genmatch==1 && dipho_sublead_genmatch==1) && (dipho_leadIDMVA > -0.2 && dipho_subleadIDMVA > -0.2) ) hist_[i]->Fill(dipho_mass, wt*lumi); else continue;}
       	   }
 }	
      }
...
//  c1->SetLogy();
  ...
  stack1->GetYaxis()->SetRangeUser(0, ymax*1.2);
  stack1->GetYaxis()->SetTitle("Events");
  stack1->GetYaxis()->SetTitleSize(20);
  stack1->GetYaxis()->SetTitleFont(43);
  stack1->GetYaxis()->SetLabelFont(43);
  stack1->GetYaxis()->SetLabelSize(15);
