...
//      tree->SetBranchAddress("dijet_dphi", &dijet_dphi);
    ...
    hist_[i] = new TH1F("hist", "", 50, 0.0, 1000.0 );
...
{ if (dipho_leadIDMVA > -0.2 && dipho_subleadIDMVA > -0.2) hist_[i]->Fill(dijet_Mjj,1); else continue;}
                
else if (File_Names[i]=="output_data_driven_bkg_2017.root") {hist_[i]->Fill(dijet_Mjj, wt);}

else if (File_Names[i]=="output_ggjets_2017.root") { hist_[i]->Fill(dijet_Mjj, wt*lumi);}
