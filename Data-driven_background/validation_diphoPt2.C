...
//      tree->SetBranchAddress("dijet_dphi", &dijet_dphi);
    ...
    hist_[i] = new TH1F("hist", "", 50, 0.0, 100.0 );
...
{ if (dipho_leadIDMVA > -0.2 && dipho_subleadIDMVA > -0.2) hist_[i]->Fill(dipho_subleadPt,1); else continue;}
                
else if (File_Names[i]=="output_data_driven_bkg_2017.root") {hist_[i]->Fill(dipho_subleadPt, wt);}

else if (File_Names[i]=="output_ggjets_2017.root") { hist_[i]->Fill(dipho_subleadPt, wt*lumi);}

else {if (!(dipho_lead_genmatch==1 && dipho_sublead_genmatch==1) && (dipho_lead_genmatch==1 || dipho_sublead_genmatch==1) && (dipho_leadIDMVA > -0.2 && dipho_subleadIDMVA > -0.2)) {hist_[i]->Fill(dipho_subleadPt, wt*lumi);}}
...
//  c2->SetTopMargin(0);
  ...
  rp1->GetXaxis()->SetTitle("dipho_subleadPt");
