...
void qcdfraction_ff()
{
  ...
  for (int ii=0;ii<nproc;ii++)
   {
    ...
    TFile* final_file = TFile::Open(proc_Name[ii]+"_FF_qcdfraction.root", "RECREATE");

    TFile *fdt = TFile::Open("Histogram_Data_fakefactor.root");
    TFile *fgg = TFile::Open("Histogram_ggjets_fakefactor.root");

    TH1F *hdt1 = (TH1F*)fdt->Get("h_ff_" + proc_Name[ii] +"_leading");
    TH1F *hgg1 = (TH1F*)fgg->Get("h_ff_" + proc_Name[ii] +"_leading");
    
    TH1F *hdt2 = (TH1F*)fdt->Get("h_ff_" + proc_Name[ii] +"_subleading");
    TH1F *hgg2 = (TH1F*)fgg->Get("h_ff_" + proc_Name[ii] +"_subleading");
   ...
   TLatex * text_proc2 = new TLatex(0.20,0.74,"FF");
   text_proc2->SetNDC();
  ...
  legend1->Draw();
   ...
   c1->Print("Plots/png/QCDFraction_FF_"+ proc_Name[ii] + ".png");
//   c1->Print("Plots/png/QCDFraction_FF_"+ proc_Name[ii] + ".pdf");

   final_file->Write();
   final_file->cd();
   hdt1->SetName("leading_"+ proc_Name[ii] + "_FF_qcdfraction");
   hdt2->SetName("subleading_"+ proc_Name[ii] + "_FF_qcdfraction");
   hdt1->Write();
   hdt2->Write();
   final_file->Close();
   }
